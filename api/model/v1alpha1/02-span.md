...
plus, on `generation_shaped` spans only, the model and usage/cost fields defined
in §5 and `06-usage-cost.md`.
...
| OpenInference `CHAIN` | `agent_step` if it orchestrates sub-steps, else `span` |
| OpenInference `RERANKER`, `EVALUATOR` | `span` (with `raw_kind` preserved) |
...
| Langfuse `CHAIN` | `agent_step` or `span` (as for OpenInference `CHAIN`) |
| Langfuse `EVALUATOR` | `span` |
| Langfuse `SPAN` | `span` |
...
A `timed_span` plus the generation fields (§5): `model`, `model_parameters`,
`completion_start_time`, the usage/cost fields (`06-usage-cost.md`), and the
OPTIONAL `prompt_ref` (§5.4). A span MUST be `generation_shaped` if and only if
it carries any generation field. Only `generation` and `embedding` kinds default
...
Generation fields (§5) are added on `generation_shaped` spans only.

### 4.1 Status model (Normative)

`status` is OTel-aligned: `{ code: "unset" | "ok" | "error", message?: string }`.

- `code` defaults to `"unset"`. A normalizer MUST set `"error"` when the source
  indicates failure (OTel `status.code = ERROR`; a source-specific error level).
...
- **Severity beyond error** (e.g. Langfuse `DEBUG`/`WARNING`, syslog levels) is
  ...
  original severity in `attributes` under the reserved key
  `llmobs.raw.level` (`08-data-quality.md` §4).
...
|---|---|---|---|
| `model` | string | yes | The model name as provided by the client (the "provided" model). Kernel-resolved model identity is a `06`-derived attribute, not a promoted field. |
| `model_parameters` | object | yes | Opaque map of request parameters (temperature, max_tokens, …). |
