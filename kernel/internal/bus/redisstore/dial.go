...
type Config struct {
	...
	URL string
	// MasterName + SentinelAddrs enable Sentinel high-availability: the
...
// every (re)connect so a rotated managed-cloud token is honored without a restart.
...
// credentials, wrong ACL user, or a missing-AUTH server: a permanent failure, not a
// transient one. The caller must surface/alert on it rather than re-poll
...
func Dial(ctx context.Context, cfg Config) (*redis.Client, error) {
	...
	case cfg.MasterName != "" && len(cfg.SentinelAddrs) > 0:
		rdb = redis.NewFailoverClient(&redis.FailoverOptions{
			...
			// carries the rotating managed-cloud token instead.
			Username:                   cfg.Username,
			Password:                   cfg.Password,
			SentinelPassword:           cfg.SentinelPassword,
			CredentialsProviderContext: provider,
			MaxRetries:                 5,                      // exponential retry across a failover
			...
			// keep idle pooled connections healthy so a blocking/slow read on an
			...
			// skip the CLIENT SETINFO identity handshake that some Valkey/
	...
	case cfg.URL != "":
		...
		// Managed-cloud auth: a username (ACL/IAM user) and/or a refreshing
	...
	if err := rdb.Ping(pctx).Err(); err != nil {
		_ = rdb.Close()
		// Classify at the boot boundary: bad credentials fail loud as a permanent
